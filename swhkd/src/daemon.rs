...
use backend::Backend;
use clap::Parser;
use config::{Hotkey, Value};
use evdev::{AttributeSet, InputEventKind, Key};
use nix::{
    sys::stat::{umask, Mode},
...
pub async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let keyboard_devices = backend.get_initial_devices(&arg_devices)?;
    ...
    let mut keyboard_states: HashMap<_, _> =
        keyboard_devices.into_iter().map(|path| (path, KeyboardState::new())).collect();

    if let Err(e) = backend.create_uinput_devices() {
        log::error!("Failed to create uinput devices: \nErr: {:#?}", e);
        exit(1);
    }
    ...
    loop {
        select! {
            ...
            Some((node, event)) = backend.next_event() => {
                ...
                let key = match event.kind() {
                    ...
                    InputEventKind::Switch(_) => {
                        backend.emit_switch_event(&event).unwrap();
                        continue
                    }
                    _ => {
                        backend.emit_event(&event).unwrap();
                        continue
                    }
                };
